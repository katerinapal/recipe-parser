...
import { baserecipeparserjs as BaseRecipeParser } from "./base-recipe-parser";
import ext_nodePath from "path";
...
mod_seriouseatsparserjs = BaseRecipeParser.extend({
  initialize: function() {
    BaseRecipeParser.prototype.initialize.apply(this, arguments);
    this.set('dataFile', ext_nodePath.resolve(__dirname + '/../test/data/serious-eats.js'));
  ...
  adjustQtyDescriptionsDirections: function(args, callback) {
    ...
    var retval = _.map(_.zip(descriptions, directions, quantities, measurementObjs), function(tuple) { //(value, key, list)
      ...
      if (zipDesc.toLowerCase() === 'vegetable cooking spray') {
        log_log.warn('Vegetable Cooking Spray amongst ingredients');
      }
...
export { mod_seriouseatsparserjs as seriouseatsparserjs };
