...
import backbone_Backbone from "backbone";
import natural_natural from "natural";
import { baserecipeparserjs as baserecipeparser_baserecipeparserjsjs } from "./base-recipe-parser";
import path_nodePath from "path";
...
seriouseatsparserjs_seriouseatsparserjs = baserecipeparser_baserecipeparserjsjs.extend({
  initialize: function() {
    baserecipeparser_baserecipeparserjsjs.prototype.initialize.apply(this, arguments);
    this.set('dataFile', path_nodePath.resolve(__dirname + '/../test/data/serious-eats.js'));
  ...
  postStripParentheses: function(parentheses, description, callback) {
    ...
    if (parentheses === 'Anaheim') { // BEGIN HACK
      ...
      if (matches) {
        util_utiljs.remove(matches, 0, 0); // remove the first element
  ...
  postProcessSplitOnAnd: function(descriptions, callback) {
    ...
    if (descriptions.length > 2) {
      ...
      if (util_utiljs.regexIndexOf(descriptions[1], /^yolks\s+separated$/, 0) >= 0
          && util_utiljs.regexIndexOf(descriptions[2], /^pressed.*$/, 0) >= 0) {
      ...
      if (util_utiljs.regexIndexOf(second, /^sliced\s+thick$/, 0) >= 0) {
        retval = [ descriptions.join(' and ') ];
  ...
  postProcessSplitOnOr: function(descriptions, parentheses, callback) {
    ...
    if (descriptions.length > 2) {
      ...
      if (util_utiljs.regexIndexOf(descriptions[1], /^minced\s+to\s+paste$/, 0) >= 0) {
      ...
      else if (util_utiljs.regexIndexOf(descriptions[1], /^thawed\s+if\s+frozen/, 0) >= 0) {
  ...
  adjustQtyDescriptionsDirections: function(args, callback) {
    ...
    var retval = _.map(_.zip(descriptions, directions, quantities, measurementObjs), function(tuple) { //(value, key, list)
      ...
      if (util_utiljs.regexIndexOf(zipDesc, blackWhitePepperRe, 0) >= 0) {
      ...
      if (zipDesc.toLowerCase() === 'vegetable cooking spray') {
        log_logjs.warn('Vegetable Cooking Spray amongst ingredients');
      }
      ...
      if (measurement) {
        if (_.indexOf(util_utiljs.altMeasurementTerms, measurement.toLowerCase()) >= 0) {
          if (zipDir.alt) {
            if (!zipQty) { zipQty = '1'; }
...
export { seriouseatsparserjs_seriouseatsparserjs as seriouseatsparserjs };
