...
import ext_path_nodePath from "path";

import {
  _ as utiljs__,
  remove as utiljs_remove,
  regexIndexOf as utiljs_regexIndexOf,
  altMeasurementTerms as utiljs_altMeasurementTerms,
} from "./util";

import { warn as logjs_warn } from "./log";
// Nodejs libs.
var _ = utiljs__;
...
seriouseatsparserjs_seriouseatsparserjs = baserecipeparser_baserecipeparserjsjs.extend({
  initialize: function() {
    ...
    this.set('dataFile', ext_path_nodePath.resolve(__dirname + '/../test/data/serious-eats.js'));
  ...
  postStripParentheses: function(parentheses, description, callback) {
    ...
    if (parentheses === 'Anaheim') { // BEGIN HACK
      ...
      if (matches) {
        utiljs_remove(matches, 0, 0); // remove the first element
  ...
  postProcessSplitOnAnd: function(descriptions, callback) {
    ...
    if (descriptions.length > 2) {
      ...
      if (utiljs_regexIndexOf(descriptions[1], /^yolks\s+separated$/, 0) >= 0
          && utiljs_regexIndexOf(descriptions[2], /^pressed.*$/, 0) >= 0) {
      ...
      if (utiljs_regexIndexOf(second, /^sliced\s+thick$/, 0) >= 0) {
        retval = [ descriptions.join(' and ') ];
  ...
  postProcessSplitOnOr: function(descriptions, parentheses, callback) {
    ...
    if (descriptions.length > 2) {
      ...
      if (utiljs_regexIndexOf(descriptions[1], /^minced\s+to\s+paste$/, 0) >= 0) {
      ...
      else if (utiljs_regexIndexOf(descriptions[1], /^thawed\s+if\s+frozen/, 0) >= 0) {
  ...
  adjustQtyDescriptionsDirections: function(args, callback) {
    ...
    var retval = _.map(_.zip(descriptions, directions, quantities, measurementObjs), function(tuple) { //(value, key, list)
      ...
      if (utiljs_regexIndexOf(zipDesc, blackWhitePepperRe, 0) >= 0) {
      ...
      if (zipDesc.toLowerCase() === 'vegetable cooking spray') {
        logjs_warn('Vegetable Cooking Spray amongst ingredients');
      }
      ...
      if (measurement) {
        if (_.indexOf(utiljs_altMeasurementTerms, measurement.toLowerCase()) >= 0) {
          if (zipDir.alt) {
            if (!zipQty) { zipQty = '1'; }
