...
exports.seriouseatsparserjs = undefined;
...
var _baseRecipeParser = require("./base-recipe-parser");
...
var seriouseatsparserjs_seriouseatsparserjs;

exports.seriouseatsparserjs = seriouseatsparserjs_seriouseatsparserjs = _baseRecipeParser.baserecipeparserjs.extend({
  initialize: function initialize() {
    _baseRecipeParser.baserecipeparserjs.prototype.initialize.apply(this, arguments);
    this.set('dataFile', _path2.default.resolve(__dirname + '/../test/data/serious-eats.js'));
    ...
    this._words = ['chopped', 'cold', 'cracked', 'cooked and peeled', 'day-old', 'drained', 'finely', 'fresh', 'freshly', 'fresh-squeezed', 'frozen', 'grated', 'ground', 'hard-cooked', 'minced', 'ripe', 'roughly', 'shredded', 'sliced', 'squeezed', 'toasted'];
  },
  postStripParentheses: function postStripParentheses(parentheses, description, callback) {
    var retParentheses = parentheses,
        retDescription;

    if (parentheses === 'Anaheim') {
      // BEGIN HACK
      // 3/4 pound Hatch (Anaheim) chiles (see note above)
      var anaheim = description.replace(parenthesesRe, '-placeholder-');
      anaheim = _.map(anaheim.split('-placeholder-'), function (item) {
        return _.trim(item);
      });

      var matches = anaheim[1].match(parenthesesRe);
      if (matches) {
        _util3.util.remove(matches, 0, 0); // remove the first element
  ...
  postProcessSplitOnAnd: function postProcessSplitOnAnd(descriptions, callback) {
    ...
    if (descriptions.length > 2) {
      ...
      if (_util3.util.regexIndexOf(descriptions[1], /^yolks\s+separated$/, 0) >= 0 && _util3.util.regexIndexOf(descriptions[2], /^pressed.*$/, 0) >= 0) {
      ...
      if (_util3.util.regexIndexOf(second, /^sliced\s+thick$/, 0) >= 0) {
        retval = [descriptions.join(' and ')];
        ...
        retval = [[fruit, 'zest', fromX].join(' '), [beg, fruit, 'juice', fromX].join(' ')];
      }
    }
  ...
  postProcessSplitOnOr: function postProcessSplitOnOr(descriptions, parentheses, callback) {
    ...
    if (descriptions.length > 2) {
      ...
      if (/^low[- ]?sodium.*$/i.test(descriptions[0]) && /^store[- ]?bought.*/i.test(descriptions[1]) && /^((chicken|vegetable) *(stock|broth))$/i.test(descriptions[2])) {

        descriptions = ['low-sodium vegetable broth', 'low-sodium chicken broth'];
      }
    } else if (descriptions.length > 1) {
      if (_util3.util.regexIndexOf(descriptions[1], /^minced\s+to\s+paste$/, 0) >= 0) {
      ...
      } else if (_util3.util.regexIndexOf(descriptions[1], /^thawed\s+if\s+frozen/, 0) >= 0) {
  ...
  adjustQtyDescriptionsDirections: function adjustQtyDescriptionsDirections(args, callback) {
    args || (args = {});
    ...
    var retval = _.map(_.zip(descriptions, directions, quantities, measurementObjs), function (tuple) {
      //(value, key, list)
      ...
      if (_util3.util.regexIndexOf(zipDesc, blackWhitePepperRe, 0) >= 0) {
      ...
      _.each(descriptionMatches, function (descObj) {
        if (descObj.re.test(zipDesc)) {
          ...
          var matchReplace = function matchReplace() {
      ...
      if (zipDesc.toLowerCase() === 'vegetable cooking spray') {
        _log.log.warn('Vegetable Cooking Spray amongst ingredients');
      }
      ...
      if (measurement) {
        if (_.indexOf(_util3.util.altMeasurementTerms, measurement.toLowerCase()) >= 0) {
          if (zipDir.alt) {
            if (!zipQty) {
              zipQty = '1';
            }
...
var endsWithItemFunc = function endsWithItemFunc(desc) {
...
var descriptionMatches = [{
  ...
  dirRe: /^(fresh|freshly squeezed)$/i,
...
exports.seriouseatsparserjs = seriouseatsparserjs_seriouseatsparserjs;