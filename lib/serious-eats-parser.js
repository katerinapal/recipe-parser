...
var _baseRecipeParser2 = _interopRequireDefault(_baseRecipeParser);
...
exports.default = _baseRecipeParser2.default.extend({
  initialize: function initialize() {
    _baseRecipeParser2.default.prototype.initialize.apply(this, arguments);
  ...
  postStripParentheses: function postStripParentheses(parentheses, description, callback) {
    ...
    if (parentheses === 'Anaheim') {
      ...
      if (matches) {
        _util4.default.remove(matches, 0, 0); // remove the first element
  ...
  postProcessSplitOnAnd: function postProcessSplitOnAnd(descriptions, callback) {
    ...
    if (descriptions.length > 2) {
      ...
      if (_util4.default.regexIndexOf(descriptions[1], /^yolks\s+separated$/, 0) >= 0 && _util4.default.regexIndexOf(descriptions[2], /^pressed.*$/, 0) >= 0) {
      ...
      if (_util4.default.regexIndexOf(second, /^sliced\s+thick$/, 0) >= 0) {
        retval = [descriptions.join(' and ')];
  ...
  postProcessSplitOnOr: function postProcessSplitOnOr(descriptions, parentheses, callback) {
    ...
    if (descriptions.length > 2) {
      ...
      if (_util4.default.regexIndexOf(descriptions[1], /^minced\s+to\s+paste$/, 0) >= 0) {
      ...
      } else if (_util4.default.regexIndexOf(descriptions[1], /^thawed\s+if\s+frozen/, 0) >= 0) {
  ...
  adjustQtyDescriptionsDirections: function adjustQtyDescriptionsDirections(args, callback) {
    ...
    var retval = _.map(_.zip(descriptions, directions, quantities, measurementObjs), function (tuple) {
      ...
      if (_util4.default.regexIndexOf(zipDesc, blackWhitePepperRe, 0) >= 0) {
      ...
      if (zipDesc.toLowerCase() === 'vegetable cooking spray') {
        _log2.default.warn('Vegetable Cooking Spray amongst ingredients');
      }
      ...
      if (measurement) {
        if (_.indexOf(_util4.default.altMeasurementTerms, measurement.toLowerCase()) >= 0) {
          if (zipDir.alt) {
            if (!zipQty) {
              zipQty = '1';
            }
