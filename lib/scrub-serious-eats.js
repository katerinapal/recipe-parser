...
import { CATEGORIES as macgourmetconstants_CATEGORIES } from "./mac-gourmet-constants";
...
var addSummary = function($, obj) {
  ...
  listHelper($, '.hrecipe .content-unit .summary p', function(index, summary) {
    ...
    text = utiljs_substituteDegree(utiljs_substituteFraction(_.trim(entities(this.html()))));
    log.ok(text);
...
var addIngredients = function(parser, $, obj) {
  ...
  _.each(categories, function(cat) {
    obj.categories.push({
      id: macgourmetconstants_CATEGORIES[cat.id],
      name: cat.id
    });
