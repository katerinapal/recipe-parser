...
var addSummary = function addSummary($, obj) {
  ...
  listHelper($, '.hrecipe .content-unit .summary p', function (index, summary) {
    ...
    text = _util.util.substituteDegree(_util.util.substituteFraction(_.trim(entities_decodejs(this.html()))));
    _log.log.ok(text);
...
var scrape_scrape = function scrape_scrape(callback, parser, url) {
