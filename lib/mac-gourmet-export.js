...
import {
  ...
  trimMultipleWhiteSpace as utiljs_trimMultipleWhiteSpace,
} from "./util";
...
mod_macgourmetexportjs = ext_Backbone.Model.extend({
  initialize: function() {
  },
  exportRecipe: function(item, source) {
    ...
    _.each(item.procedures, function(procedure) {
      ...
      if (procText) {
        procText = utiljs_trimMultipleWhiteSpace(procText);
        procObj = {
          VARIATION_ID: -1,
    ...
    (function walker(array, list, isTop) {
      if (_.isArray(array)) {
        ...
        tmp = utiljs_substituteDegree(utiljs_substituteFraction(tmp));
    ...
    obj['TYPE'] = 102;
    obj['URL'] = ext_URL.format(item.parsedUrl);
...
var mod_macgourmetexportjs;
export { mod_macgourmetexportjs as macgourmetexportjs };
