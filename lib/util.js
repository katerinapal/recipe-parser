...
exports.linefeed = util_linefeed = process.platform === 'win32' ? '\r\n' : '\n';

// Normalize linefeeds in a string.
util_normalizelf = function util_normalizelf(str) {
  return str.replace(/\r\n|\n/g, util_linefeed);
};

exports.remove = util_remove = function util_remove(array, from, to) {
...
util_calcPadding = function util_calcPadding(num, minLen) {
...
exports.substituteDegree = util_substituteDegree = function util_substituteDegree(str) {
  if (!str) {
    return '';
  }
...
exports.substituteFraction = util_substituteFraction = function util_substituteFraction(str) {
  if (!str) {
    return '';
  }
...
exports.decodeFractions = util_decodeFractions = function util_decodeFractions(str) {
...
exports.expandHomeDir = util_expandHomeDir = function util_expandHomeDir(dir) {
  dir = dir || '';
  if (dir.indexOf('~') === 0) {
...
util_descSortByStr = function util_descSortByStr(obj, val, context) {
...
var text = exports.text = util_text = function util_text(elem) {
  ...
  return (0, _entities.decode)(text);
};
...
exports.trimMultipleWhiteSpace = util_trimMultipleWhiteSpace = function util_trimMultipleWhiteSpace(text) {
  return text.replace(whiteSpaceRe, ' '); // replace extra spaces with one
};

// http://stackoverflow.com/questions/273789
exports.regexIndexOf = util_regexIndexOf = function util_regexIndexOf(str, regex, startpos) {
...
exports.altMeasurementTerms = util_altMeasurementTerms = _underscore2.default.union(_altMeasurementTerms, _underscore2.default.map(_altMeasurementTerms, function (term) {
  return _pluralize2.default.plural(term);
})).sort();

_util_saveIngredients = function util_saveIngredients(items, fileName) {
  var data = require(fileName);
  _underscore2.default.each(items, function (item) {
    _underscore2.default.each(_util_saveIngredients, function (ingredient) {
...
util_saveJustIngredients = function util_saveJustIngredients(ingredients, fileName, callback) {
  var data = require(fileName);
...
// collate all images
util_collateAllImages = function util_collateAllImages(items) {
  var images = [];
  _underscore2.default.each(items, function (item) {
...
// download all images
util_downloadAllImages = function util_downloadAllImages(images, callback) {
...
util_savePlistToFile = function util_savePlistToFile(obj) {
...
exports.util = util;
exports._ = util__;
exports.linefeed = util_linefeed;
exports.remove = util_remove;
exports.repeat = util_repeat;
exports.substituteDegree = util_substituteDegree;
exports.substituteFraction = util_substituteFraction;
exports.decodeFractions = util_decodeFractions;
exports.writePlist = util_writePlist;
exports.expandHomeDir = util_expandHomeDir;
exports.unescape = util_unescape;
exports.text = util_text;
exports.fulltext = util_fulltext;
exports.splitCSS = util_splitCSS;
exports.trimMultipleWhiteSpace = util_trimMultipleWhiteSpace;
exports.regexIndexOf = util_regexIndexOf;
exports.altMeasurementTerms = util_altMeasurementTerms;