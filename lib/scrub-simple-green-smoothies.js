...
import ext_nodeio_nodeio from "node.io";
...
import {
  ...
  writePlist as utiljs_writePlist,
  expandHomeDir as utiljs_expandHomeDir,
  unescape as utiljs_unescape,
} from "./util";
...
var verbose = function() {
  if (ext_commander_program.debug) {
...
var addSummary = function($, obj) {
  ...
  listHelper($, '.entry span[itemprop="description"]', false, function(ele) {
    ...
    if (summary) {
      obj.summaries.push(utiljs_substituteFraction(util_utiljs.trim(summary)));
    }
  });
};
...
var addProcedure = function($, obj) {
  ...
  listHelper($, '.entry span[itemprop="recipeInstructions"]', true, function(procedure) {
    if (!procedure) { return; }
    obj.procedures.push(utiljs_substituteDegree(utiljs_substituteFraction(util_utiljs.trim(procedure.striptags || procedure.innerHTML))));
  });
};
...
var addIngredients = function($, obj) {
  ...
  _.each(ingredients, function(ingredient) {
    breakdown = {};
    text = utiljs_unescape(ingredient);
    if (text) {
      ...
      if (matches && matches.length) {
        ...
        if (matches[3].indexOf(',') > 0) {
          ...
          breakdown.product = utiljs_substituteFraction(util_utiljs.trim(matches[1]));
          breakdown.direction = utiljs_substituteFraction(util_utiljs.trim(matches[2]));
        } else {
          breakdown.product = utiljs_substituteFraction(util_utiljs.trim(matches[3]));
        }
...
var scrape = function(callback, url) {
  ...
  var job = new ext_nodeio_nodeio.Job({
    auto_retry: true,
    timeout: 20,
    retries: 3,
    silent: true
  ...
  ext_nodeio_nodeio.start(job, {}, function(err, data) {
    if (err) { callback(err); }
    callback(null, data);
  }, true);
};

if (ext_commander_program.url) {
  var url = ext_commander_program.url;
  ...
  var exportRecipe = function(item) {
    ...
    var plist_file = utiljs_expandHomeDir('~/Desktop/recipe.mgourmet4');
    utiljs_writePlist(function(err, obj) {
  ...
  scrape(function(err, items) {
    ...
    ext_async_async.forEach(items, function(item, done) {
      if (item.image.src) {
        var oURL = ext_url_URL.parse(item.image.src),
            request = ext_http_http.request({
              port: 80,
              host: oURL.hostname,
              method: 'GET',
              path: oURL.pathname
  ...
  console.log(ext_commander_program.description());
  console.log("Version: " + ext_commander_program.version());
  console.log(ext_commander_program.helpInformation());
}
