...
import ext_nodeio from "node.io";
...
var addSummary = function($, obj) {
  ...
  listHelper($, '.entry span[itemprop="description"]', false, function(ele) {
    ...
    if (summary) {
      obj.summaries.push(utiljs_substituteFraction(util.trim(summary)));
    }
  });
};
...
var addProcedure = function($, obj) {
  ...
  listHelper($, '.entry span[itemprop="recipeInstructions"]', true, function(procedure) {
    if (!procedure) { return; }
    obj.procedures.push(utiljs_substituteDegree(utiljs_substituteFraction(util.trim(procedure.striptags || procedure.innerHTML))));
  });
};
    ...
    if (text) {
      ...
      if (matches && matches.length) {
        ...
        if (matches[3].indexOf(',') > 0) {
          ...
          breakdown.product = utiljs_substituteFraction(util.trim(matches[1]));
          breakdown.direction = utiljs_substituteFraction(util.trim(matches[2]));
        } else {
          breakdown.product = utiljs_substituteFraction(util.trim(matches[3]));
        }
...
var scrape = function(callback, url) {
  ...
  ext_nodeio.start(job, {}, function(err, data) {
    if (err) { callback(err); }
    callback(null, data);
  }, true);
};
