...
var addSummary = function($, obj) {
  ...
  listHelper($, '.entry span[itemprop="description"]', false, function(ele) {
    ...
    if (summary) {
      obj.summaries.push(utiljs_substituteFraction(util_util.trim(summary)));
    }
  });
};
...
var addProcedure = function($, obj) {
  ...
  listHelper($, '.entry span[itemprop="recipeInstructions"]', true, function(procedure) {
    if (!procedure) { return; }
    obj.procedures.push(utiljs_substituteDegree(utiljs_substituteFraction(util_util.trim(procedure.striptags || procedure.innerHTML))));
  });
};
    ...
    if (text) {
      ...
      if (matches && matches.length) {
        ...
        if (matches[3].indexOf(',') > 0) {
          ...
          breakdown.product = utiljs_substituteFraction(util_util.trim(matches[1]));
          breakdown.direction = utiljs_substituteFraction(util_util.trim(matches[2]));
        } else {
          breakdown.product = utiljs_substituteFraction(util_util.trim(matches[3]));
        }
...
if (ext_program.url) {
  ...
  var exportRecipe = function(item) {
    ...
    obj['NAME'] = util_util.trim(item.title);
    obj['NOTE'] = '';
    obj['NOTES_LIST'] = [];
    obj['NUTRITION'] = '';
    ...
    obj['TYPE'] = 102;
    obj['URL'] = url;
    obj['YIELD'] = util_util.trim(item.servings);
    ...
    _.each(item.procedures, function(procedure) {
      procedure = util_util.trim(procedure);
      if (procedure) {
    ...
    _.each(item.ingredients, function(ingredient) {
      ingredients.push({
        DESCRIPTION: util_util.trim(ingredient.product),
        DIRECTION: util_util.trim(ingredient.direction) || '',
        INCLUDED_RECIPE_ID: -1,
        IS_DIVIDER: false,
        IS_MAIN: false,
        MEASUREMENT: util_util.trim(ingredient.measurement),
        QUANTITY: util_util.trim(ingredient.quantity)
      });
    });
  ...
  scrape(function(err, items) {
    ...
    ext_async_async.forEach(items, function(item, done) {
      if (item.image.src) {
        var oURL = ext_URL.parse(item.image.src),
  ...
  console.log(ext_program.description());
  console.log("Version: " + ext_program.version());
  console.log(ext_program.helpInformation());
}
