...
var addSummary = function addSummary($, obj) {
  ...
  listHelper($, '.entry span[itemprop="description"]', false, function (ele) {
    ...
    if (summary) {
      obj.summaries.push(_util4.default.substituteFraction(_util4.default.trim(summary)));
    }
  });
};
...
var addProcedure = function addProcedure($, obj) {
  ...
  listHelper($, '.entry span[itemprop="recipeInstructions"]', true, function (procedure) {
    ...
    obj.procedures.push(_util4.default.substituteDegree(_util4.default.substituteFraction(_util4.default.trim(procedure.striptags || procedure.innerHTML))));
  });
};
...
var addIngredients = function addIngredients($, obj) {
  ...
  _.each(ingredients, function (ingredient) {
    breakdown = {};
    text = _util4.default.unescape(ingredient);
    if (text) {
      ...
      if (matches && matches.length) {
        ...
        if (matches[3].indexOf(',') > 0) {
          ...
          breakdown.product = _util4.default.substituteFraction(_util4.default.trim(matches[1]));
          breakdown.direction = _util4.default.substituteFraction(_util4.default.trim(matches[2]));
        } else {
          breakdown.product = _util4.default.substituteFraction(_util4.default.trim(matches[3]));
        }
...
if (_commander2.default.url) {
  ...
  var exportRecipe = function exportRecipe(item) {
    ...
    obj['NAME'] = _util4.default.trim(item.title);
    obj['NOTE'] = '';
    obj['NOTES_LIST'] = [];
    obj['NUTRITION'] = '';
    ...
    obj['TYPE'] = 102;
    obj['URL'] = url;
    obj['YIELD'] = _util4.default.trim(item.servings);
    ...
    _.each(item.procedures, function (procedure) {
      procedure = _util4.default.trim(procedure);
      if (procedure) {
    ...
    _.each(item.ingredients, function (ingredient) {
      ingredients.push({
        DESCRIPTION: _util4.default.trim(ingredient.product),
        DIRECTION: _util4.default.trim(ingredient.direction) || '',
        INCLUDED_RECIPE_ID: -1,
        IS_DIVIDER: false,
        IS_MAIN: false,
        MEASUREMENT: _util4.default.trim(ingredient.measurement),
        QUANTITY: _util4.default.trim(ingredient.quantity)
      });
    });

    var plist_file = _util4.default.expandHomeDir('~/Desktop/recipe.mgourmet4');
    _util4.default.writePlist(function (err, obj) {
      if (err) {
        console.error(err);
      }
    }, [obj], plist_file);
