...
import nodeio_nodeio from "node.io";
import commander_program from "commander";
...
var addSummary = function($, obj) {
  ...
  listHelper($, '.entry span[itemprop="description"]', false, function(ele) {
    ...
    if (summary) {
      obj.summaries.push(util_utiljs.substituteFraction(util_utiljs.trim(summary)));
    }
  });
};
...
var addProcedure = function($, obj) {
  ...
  listHelper($, '.entry span[itemprop="recipeInstructions"]', true, function(procedure) {
    if (!procedure) { return; }
    obj.procedures.push(util_utiljs.substituteDegree(util_utiljs.substituteFraction(util_utiljs.trim(procedure.striptags || procedure.innerHTML))));
  });
};
...
var addIngredients = function($, obj) {
  ...
  _.each(ingredients, function(ingredient) {
    breakdown = {};
    text = util_utiljs.unescape(ingredient);
    if (text) {
      ...
      if (matches && matches.length) {
        ...
        if (matches[3].indexOf(',') > 0) {
          ...
          breakdown.product = util_utiljs.substituteFraction(util_utiljs.trim(matches[1]));
          breakdown.direction = util_utiljs.substituteFraction(util_utiljs.trim(matches[2]));
        } else {
          breakdown.product = util_utiljs.substituteFraction(util_utiljs.trim(matches[3]));
        }
...
var scrape = function(callback, url) {
  ...
  nodeio_nodeio.start(job, {}, function(err, data) {
    if (err) { callback(err); }
    callback(null, data);
  }, true);
};

if (commander_program.url) {
  var url = commander_program.url;
  ...
  var exportRecipe = function(item) {
    ...
    obj['NAME'] = util_utiljs.trim(item.title);
    obj['NOTE'] = '';
    obj['NOTES_LIST'] = [];
    obj['NUTRITION'] = '';
    ...
    obj['TYPE'] = 102;
    obj['URL'] = url;
    obj['YIELD'] = util_utiljs.trim(item.servings);
    ...
    _.each(item.procedures, function(procedure) {
      procedure = util_utiljs.trim(procedure);
      if (procedure) {
    ...
    _.each(item.ingredients, function(ingredient) {
      ingredients.push({
        DESCRIPTION: util_utiljs.trim(ingredient.product),
        DIRECTION: util_utiljs.trim(ingredient.direction) || '',
        INCLUDED_RECIPE_ID: -1,
        IS_DIVIDER: false,
        IS_MAIN: false,
        MEASUREMENT: util_utiljs.trim(ingredient.measurement),
        QUANTITY: util_utiljs.trim(ingredient.quantity)
      });
    });

    var plist_file = util_utiljs.expandHomeDir('~/Desktop/recipe.mgourmet4');
    util_utiljs.writePlist(function(err, obj) {
  ...
  scrape(function(err, items) {
    ...
    async_async.forEach(items, function(item, done) {
      if (item.image.src) {
        var oURL = url_URL.parse(item.image.src),
            request = http_http.request({
              port: 80,
              host: oURL.hostname,
              method: 'GET',
              path: oURL.pathname
  ...
  console.log(commander_program.description());
  console.log("Version: " + commander_program.version());
  console.log(commander_program.helpInformation());
}
