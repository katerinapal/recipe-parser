...
var addSummary = function addSummary($, obj) {
  ...
  listHelper($, '.entry span[itemprop="description"]', false, function (ele) {
    ...
    if (summary) {
      obj.summaries.push(_util3.util.substituteFraction(_util3.util.trim(summary)));
    }
  });
};
...
var addProcedure = function addProcedure($, obj) {
  ...
  listHelper($, '.entry span[itemprop="recipeInstructions"]', true, function (procedure) {
    ...
    obj.procedures.push(_util3.util.substituteDegree(_util3.util.substituteFraction(_util3.util.trim(procedure.striptags || procedure.innerHTML))));
  });
};
...
var addIngredients = function addIngredients($, obj) {
  ...
  _.each(ingredients, function (ingredient) {
    breakdown = {};
    text = _util3.util.unescape(ingredient);
    if (text) {
      ...
      if (matches && matches.length) {
        ...
        if (matches[3].indexOf(',') > 0) {
          ...
          breakdown.product = _util3.util.substituteFraction(_util3.util.trim(matches[1]));
          breakdown.direction = _util3.util.substituteFraction(_util3.util.trim(matches[2]));
        } else {
          breakdown.product = _util3.util.substituteFraction(_util3.util.trim(matches[3]));
        }
...
if (_commander2.default.url) {
  ...
  var exportRecipe = function exportRecipe(item) {
    ...
    obj['NAME'] = _util3.util.trim(item.title);
    obj['NOTE'] = '';
    obj['NOTES_LIST'] = [];
    obj['NUTRITION'] = '';
    ...
    obj['TYPE'] = 102;
    obj['URL'] = url;
    obj['YIELD'] = _util3.util.trim(item.servings);
    ...
    _.each(item.procedures, function (procedure) {
      procedure = _util3.util.trim(procedure);
      if (procedure) {
    ...
    _.each(item.ingredients, function (ingredient) {
      ingredients.push({
        DESCRIPTION: _util3.util.trim(ingredient.product),
        DIRECTION: _util3.util.trim(ingredient.direction) || '',
        INCLUDED_RECIPE_ID: -1,
        IS_DIVIDER: false,
        IS_MAIN: false,
        MEASUREMENT: _util3.util.trim(ingredient.measurement),
        QUANTITY: _util3.util.trim(ingredient.quantity)
      });
    });

    var plist_file = _util3.util.expandHomeDir('~/Desktop/recipe.mgourmet4');
    _util3.util.writePlist(function (err, obj) {
      if (err) {
        console.error(err);
      }
    }, [obj], plist_file);
