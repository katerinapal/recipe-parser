...
var addSummary = function addSummary($, obj) {
  ...
  listHelper($, '.entry span[itemprop="description"]', false, function (ele) {
    ...
    if (summary) {
      obj.summaries.push((0, _util3.substituteFraction)(_util3.utiljs.trim(summary)));
    }
  });
};
...
var addProcedure = function addProcedure($, obj) {
  ...
  listHelper($, '.entry span[itemprop="recipeInstructions"]', true, function (procedure) {
    ...
    obj.procedures.push((0, _util3.substituteDegree)((0, _util3.substituteFraction)(_util3.utiljs.trim(procedure.striptags || procedure.innerHTML))));
  });
};
...
var addIngredients = function addIngredients($, obj) {
  ...
  _.each(ingredients, function (ingredient) {
    breakdown = {};
    text = (0, _util3.unescape)(ingredient);
    if (text) {
      ...
      if (matches && matches.length) {
        ...
        if (matches[3].indexOf(',') > 0) {
          ...
          breakdown.product = (0, _util3.substituteFraction)(_util3.utiljs.trim(matches[1]));
          breakdown.direction = (0, _util3.substituteFraction)(_util3.utiljs.trim(matches[2]));
        } else {
          breakdown.product = (0, _util3.substituteFraction)(_util3.utiljs.trim(matches[3]));
        }
...
if (_commander2.default.url) {
  ...
  var exportRecipe = function exportRecipe(item) {
    ...
    obj['NAME'] = _util3.utiljs.trim(item.title);
    obj['NOTE'] = '';
    obj['NOTES_LIST'] = [];
    obj['NUTRITION'] = '';
    ...
    obj['TYPE'] = 102;
    obj['URL'] = url;
    obj['YIELD'] = _util3.utiljs.trim(item.servings);
    ...
    _.each(item.procedures, function (procedure) {
      procedure = _util3.utiljs.trim(procedure);
      if (procedure) {
    ...
    _.each(item.ingredients, function (ingredient) {
      ingredients.push({
        DESCRIPTION: _util3.utiljs.trim(ingredient.product),
        DIRECTION: _util3.utiljs.trim(ingredient.direction) || '',
        INCLUDED_RECIPE_ID: -1,
        IS_DIVIDER: false,
        IS_MAIN: false,
        MEASUREMENT: _util3.utiljs.trim(ingredient.measurement),
        QUANTITY: _util3.utiljs.trim(ingredient.quantity)
      });
    });

    var plist_file = (0, _util3.expandHomeDir)('~/Desktop/recipe.mgourmet4');
    (0, _util3.writePlist)(function (err, obj) {
      if (err) {
        console.error(err);
      }
    }, [obj], plist_file);
