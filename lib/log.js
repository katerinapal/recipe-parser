...
import { _ as utiljs__, repeat as utiljs_repeat } from "./util";
import ext_colors_colors from "colors";
// Nodejs libs.
var _ = utiljs__;
...
function format(args) {
  ...
  return ext_util_nodeUtil.format.apply(ext_util_nodeUtil, args);
}

function write(msg) {
  msg = msg || '';
  // Actually write output.
  if (!log_muted && !suppressOutput) {
    hasLogged = true;
...
log_warn = function() {
  ...
  if (arguments.length > 0) {
    writeln('>> '.red + _.trim(msg).replace(/\n/g, '\n>> '.red));
  } else {
    writeln('ERROR'.red);
  }
  return log;
...
log_error = function() {
  //main.fail.errorcount++;
  log_warn.apply(log, arguments);
  return log;
};
log_ok = function() {
  var msg = format(arguments);
  if (arguments.length > 0) {
    writeln('>> '.green + _.trim(msg).replace(/\n/g, '\n>> '.green));
  } else {
    writeln('OK'.green);
  }
  return log;
};
log_errorlns = function() {
  var msg = format(arguments);
  log_error(log_wraptext(77, msg));
  return log;
};
log_oklns = function() {
  var msg = format(arguments);
  log_ok(log_wraptext(77, msg));
  return log;
};
...
log_writetableln = function(widths, texts) {
  writeln(log_table(widths, texts));
  return log;
};

// Wrap a long line of text to 80 columns.
log_writelns = function() {
  var msg = format(arguments);
  writeln(log_wraptext(80, msg));
  return log;
};
...
Object.keys(log).filter(function(key) {
  ...
  // IS NOT set.
  log_verbose[key] = function() {
    suppressOutput = true; //!RecipeParser.options.verbose;
  ...
  // Like any other log function, but suppresses output if the "verbose" option
  // IS set.
  log_notverbose[key] = function() {
    suppressOutput = false; //RecipeParser.options.verbose;
    log[key].apply(log, arguments);
    suppressOutput = false;
    return log_notverbose;
  };
});
...
log_verbose.or = log_notverbose;
log_notverbose.or = log_verbose;

// Static methods.

// Pretty-format a word list.
log_wordlist = function(arr, options) {
...
log_table = function(widths, texts) {
  var rows = [];
  widths.forEach(function(width, i) {
    var lines = log_wraptext(width, texts[i]).split('\n');
  ...
  rows.forEach(function(row) {
    ...
    for (var i = 0; i < row.length; i++) {
      column = row[i] || '';
      txt += column;
      var diff = widths[i] - log_uncolor(column).length;
      if (diff > 0) { txt += utiljs_repeat(diff, ' '); }
    }
    lines.push(txt);
  });
...
export { log_warn as warn, log_ok as ok, log_errorlns as errorlns, log_oklns as oklns, log_debug as debug, log_writelns as writelns };
