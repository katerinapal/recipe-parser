...
var FoodNetworkRecipeParser = baserecipeparser_baserecipeparserjsjs.extend({
  initialize: function() {
    baserecipeparser_baserecipeparserjsjs.prototype.initialize.apply(this, arguments);
    this.set('dataFile', path_nodePath.resolve(__dirname + '/../test/data/food-network.js'));
  ...
  postProcessSplitOnAnd: function(descriptions, callback) {
    ...
    if (descriptions.length > 2) {
      ...
      if (util_utiljs.regexIndexOf(descriptions[1], /^sliced\s+thick$/, 0) >= 0) {
        retval = [descriptions.join(' and ')];
      }
    }
...
foodnetworkparser_FoodNetworkRecipeParser = FoodNetworkRecipeParser;
export { foodnetworkparser_FoodNetworkRecipeParser as FoodNetworkRecipeParser };
