...
var constants = _macGourmetConstants.macgourmetconstantsjs.constants,
...
var addImage = function addImage($, obj) {
  ...
  listHelper($, '.recipe-image .image', function (index, img) {
    var backgroundImage = _.first((0, _util.splitCSS)(this.css('background')));
    match = patternRe.exec(backgroundImage);

    if (match && match.length) {
      _log.logjs.ok(match[1]);
      obj.image = {
        src: match[1]
      };
    } else {
      _log.logjs.ok(backgroundImage);
      obj.image = {
        src: backgroundImage
      };
    }
...
var addSummary = function addSummary($, obj) {
  ...
  listHelper($, '.why .full p', function (index, summary) {
    ...
    text = (0, _util.substituteDegree)((0, _util.substituteFraction)(_.trim((0, _util.fulltext)(summary))));
    _log.logjs.ok(index + 1 + '- ' + text);
...
var addProcedures = function addProcedures($, obj) {
  ...
  listHelper($, '.instructions ol li[itemprop="recipeInstructions"] div', function (index, procedure) {
    ...
    text = (0, _util.substituteDegree)((0, _util.substituteFraction)(_.trim((0, _util.fulltext)(procedure))));
...
var addNotes = function addNotes($, obj) {
  ...
  listHelper($, '.serves > p', function (index, note) {
    text = (0, _util.substituteDegree)((0, _util.substituteFraction)(_.trim((0, _util.fulltext)(note))));
...
var addCourse = function addCourse($, obj) {
  ...
  listHelper($, '.other-attributes meta[itemprop="recipeCategory"]', function (index, meta) {
    ...
    if (obj.course) {
      _log.logjs.oklns(obj.course.id + ' - ' + obj.course.name);
    }
  });
};
...
var addAsideNotes = function addAsideNotes($, obj) {
  ...
  listHelper($, '.asides > .aside', function (i, aside) {
    listHelper($, 'h4', this, function () {
      h4 = _.trim((0, _util.text)(this));
    });
    listHelper($, 'h3', this, function () {
      h3 = _.trim((0, _util.text)(this));
    });
    ...
    $(this).children('p').each(function (index, element) {
      text = (0, _util.substituteDegree)((0, _util.substituteFraction)(_.trim((0, _util.text)(this))));
      if (text) {
    ...
    listHelper($, '.page-item', this, function () {
      ...
      listHelper($, 'figure > figcaption', this, function () {
        text = (0, _util.substituteDegree)((0, _util.substituteFraction)(_.trim((0, _util.fulltext)(this))));
        match = text.match(removeLeadingDigitPeriodRe);
        if (match) {
          text = match[1];
        }
    ...
    _log.logjs.ok(note.h4 + ' : ' + note.h3);
    _.each(note.notes, function (note, i) {
      _log.logjs.ok(i + 1 + '- ' + note.text);
      _log.logjs.ok(i + 1 + '- ' + note.image.src);
    });
  });
};
...
var combineNotes = function combineNotes(obj) {
  ...
  _.each(asideNotes, function (asideNote) {
    ...
    if (intros && intros.length) {
      dest.push({
        text: title.concat(intros).join(_util.linefeed),
        order: i++,
        kind: 10
      });
    ...
    _.each(asideNote.notes, function (note) {
      noteObj = {
        text: title.concat(['<p>' + note.text + '</p>']).join(_util.linefeed),
        order: i++,
        kind: 10
      };
      title = []; // reset the <h4> and <h3>
