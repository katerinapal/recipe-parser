...
var addCourse = function($, obj) {
  ...
  listHelper($, '.other-attributes meta[itemprop="recipeCategory"]', function(index, meta) {
    ...
    if (name) {
      ...
      if (val) {
        obj.course = {
          id: macgourmetconstants_constants.COURSES[val],
          name: val
        };
      }
      ...
      if (cat) {
        obj.categories.push({
          id: macgourmetconstants_constants.CATEGORIES[cat],
          name: cat
        });
      }
    }
    ...
    if (obj.course) {
      log_log.oklns(obj.course.id + ' - ' + obj.course.name);
    }
  });
};
...
var addAsideNotes = function($, obj) {
  ...
  listHelper($, '.asides > .aside', function(i, aside) {
    ...
    log_log.ok(note.h4 + ' : ' + note.h3);
    _.each(note.notes, function(note, i) {
      log_log.ok(i + 1 + '- ' + note.text);
      log_log.ok(i + 1 + '- ' + note.image.src);
    });
  });
};
