...
var constants = _macGourmetConstants.constants,
...
var addIngredients = function addIngredients(parser, $, obj) {
    ...
    if (this.attr('itemprop') === 'ingredients') {
      ...
      (function walker(vals) {
        if (_.isArray(vals)) {
          ...
          _log.log.ok((0, _util.substituteDegree)((0, _util.substituteFraction)(output)));
        }
      })(retval);
      ...
      listHelper($, 'h5', this, function () {
        _log.log.ok('Group: ' + _.trim((0, _util.text)(this)));
        var parent = {
  ...
  _.each(categories, function (cat) {
    obj.categories.push({
      id: _macGourmetConstants.constants.CATEGORIES[cat.id],
      name: cat.id
    });
...
var addCourse = function addCourse($, obj) {
  ...
  listHelper($, '.other-attributes meta[itemprop="recipeCategory"]', function (index, meta) {
    ...
    if (name) {
      ...
      if (val) {
        obj.course = {
          id: _macGourmetConstants.constants.COURSES[val],
          name: val
        };
      }
      ...
      if (cat) {
        obj.categories.push({
          id: _macGourmetConstants.constants.CATEGORIES[cat],
          name: cat
        });
      }
    }
