...
var _categoryClassifier2 = _interopRequireDefault(_categoryClassifier);
...
var classifier = new _categoryClassifier2.default(),
    _ = _util2.default._;
...
var listHelper = function listHelper($, selector, context, callback) {
  ...
  try {
    ...
    _log2.default.debug(elements.length);
    if (elements.length) {
    ...
    _log2.default.errorlns(e);
  }
};

var addTitle = function addTitle($, obj) {
  _log2.default.writelns('Adding Title');
  var text;
  ...
  listHelper($, '.recipe.content h2[itemprop="name"]', function (index, title) {
    ...
    obj.title = _.trim(_util2.default.text(this));
    _log2.default.oklns(obj.title);
  });
};

var addDatePublished = function addDatePublished($, obj) {
  _log2.default.writelns('Adding Date Published');
  listHelper($, 'meta[itemprop="datePublished"]', function (index, meta) {
    obj.datePublished = _.trim(this.attr('content'));
    _log2.default.oklns(obj.datePublished);
  });
};

var addServings = function addServings($, obj) {
  _log2.default.writelns('Adding Servings');
  listHelper($, 'h4[itemprop="recipeYield"]', function (index, h4) {
    obj.servings = _util2.default.substituteFraction(_.trim(_util2.default.text(this)));
    _log2.default.oklns(obj.servings);
  });
};

var addImage = function addImage($, obj) {
  obj.image || (obj.image = {});
  _log2.default.writelns('Adding Image');
  ...
  listHelper($, '.recipe-image .image', function (index, img) {
    var backgroundImage = _.first(_util2.default.splitCSS(this.css('background')));
    match = patternRe.exec(backgroundImage);

    if (match && match.length) {
      _log2.default.ok(match[1]);
      obj.image = {
        src: match[1]
      };
    } else {
      _log2.default.ok(backgroundImage);
      obj.image = {
        src: backgroundImage
      };
    }
...
var addSummary = function addSummary($, obj) {
  ...
  _log2.default.writelns('Adding Summary');
  var text;
  ...
  listHelper($, '.why .full p', function (index, summary) {
    ...
    text = _util2.default.substituteDegree(_util2.default.substituteFraction(_.trim(_util2.default.fulltext(summary))));
    _log2.default.ok(index + 1 + '- ' + text);
...
var addIngredients = function addIngredients(parser, $, obj) {
  ...
  _log2.default.writelns('Adding Ingredients');
  ...
  listHelper($, '.ingredients > ul li', function (index, ingredient) {
    if (this.attr('itemprop') === 'ingredients') {
      text = _.trim(_util2.default.fulltext(ingredient));
      ...
      (function walker(vals) {
        if (_.isArray(vals)) {
          ...
          _log2.default.oklns(vals.description);
          ...
          _log2.default.ok(_util2.default.substituteDegree(_util2.default.substituteFraction(output)));
        }
      })(retval);
      ...
      listHelper($, 'h5', this, function () {
        _log2.default.ok('Group: ' + _.trim(_util2.default.text(this)));
        var parent = {
          description: _.trim(_util2.default.text(this)),
          isDivider: true,
          ingredients: []
        };
        top.push(parent);
  ...
  _log2.default.writelns('Guessing Categories');
...
var addProcedures = function addProcedures($, obj) {
  _log2.default.writelns('Adding Procedures');
  ...
  listHelper($, '.instructions ol li[itemprop="recipeInstructions"] div', function (index, procedure) {
    ...
    listHelper($, 'b', this, function () {
      header = _.trim(_util2.default.text(this));
    });

    text = _util2.default.substituteDegree(_util2.default.substituteFraction(_.trim(_util2.default.fulltext(procedure))));
    ...
    if (header) {
      _log2.default.oklns(index + 1 + ' # ' + header + ' # ' + text);
    } else {
      _log2.default.oklns(index + 1 + ' - ' + text);
    }
  });
};

var addNotes = function addNotes($, obj) {
  _log2.default.writelns('Adding Notes');
  obj.notes || (obj.notes = []);
  var text;

  listHelper($, '.serves > p', function (index, note) {
    text = _util2.default.substituteDegree(_util2.default.substituteFraction(_.trim(_util2.default.fulltext(note))));
...
var addTimes = function addTimes($, obj) {
  _log2.default.writelns('Adding Times');
  var text;
...
var addCourse = function addCourse($, obj) {
  ...
  listHelper($, '.other-attributes meta[itemprop="recipeCategory"]', function (index, meta) {
    ...
    if (obj.course) {
      _log2.default.oklns(obj.course.id + ' - ' + obj.course.name);
    }
  });
};

var addAsideNotes = function addAsideNotes($, obj) {
  _log2.default.writelns('Adding Aside Notes');
  ...
  listHelper($, '.asides > .aside', function (i, aside) {
    listHelper($, 'h4', this, function () {
      h4 = _.trim(_util2.default.text(this));
    });
    listHelper($, 'h3', this, function () {
      h3 = _.trim(_util2.default.text(this));
    });
    ...
    $(this).children('p').each(function (index, element) {
      text = _util2.default.substituteDegree(_util2.default.substituteFraction(_.trim(_util2.default.text(this))));
      if (text) {
    ...
    listHelper($, '.page-item', this, function () {
      ...
      listHelper($, 'figure > figcaption', this, function () {
        text = _util2.default.substituteDegree(_util2.default.substituteFraction(_.trim(_util2.default.fulltext(this))));
        match = text.match(removeLeadingDigitPeriodRe);
        if (match) {
          text = match[1];
        }
    ...
    _log2.default.ok(note.h4 + ' : ' + note.h3);
    _.each(note.notes, function (note, i) {
      _log2.default.ok(i + 1 + '- ' + note.text);
      _log2.default.ok(i + 1 + '- ' + note.image.src);
    });
  });
};
...
var combineNotes = function combineNotes(obj) {
  ...
  _.each(asideNotes, function (asideNote) {
    ...
    var intros = _.map(asideNote.intros, function (intro) {
      return '<p>' + intro + '</p>';
    }).join(_util2.default.linefeed);

    if (intros && intros.length) {
      dest.push({
        text: title.concat(intros).join(_util2.default.linefeed),
        order: i++,
        kind: 10
      });
    ...
    _.each(asideNote.notes, function (note) {
      noteObj = {
        text: title.concat(['<p>' + note.text + '</p>']).join(_util2.default.linefeed),
        order: i++,
        kind: 10
      };
      title = []; // reset the <h4> and <h3>
...
var scrape = exports.scrape = function scrape(callback, parser, url) {
