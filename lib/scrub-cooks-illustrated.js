...
import { macgourmetconstantsjs as macgourmetconstants_macgourmetconstantsjs } from "./mac-gourmet-constants";
import { categoryclassifierjs as CategoryClassifier } from "./category-classifier";
...
import {
  ...
  splitCSS as utiljs_splitCSS,
} from "./util";

import { logjs as log } from "./log";

var constants = macgourmetconstants_macgourmetconstantsjs.constants, classifier = new CategoryClassifier(), _ = utiljs__;
...
var addImage = function($, obj) {
  ...
  listHelper($, '.recipe-image .image', function(index, img) {
    var backgroundImage = _.first(utiljs_splitCSS(this.css('background')));
...
var addSummary = function($, obj) {
  ...
  listHelper($, '.why .full p', function(index, summary) {
    ...
    text = utiljs_substituteDegree(utiljs_substituteFraction(_.trim(utiljs_fulltext(summary))));
    log.ok(index + 1 + '- ' + text);
    ...
    if (this.attr('itemprop') === 'ingredients') {
      ...
      (function walker(vals) {
        if (_.isArray(vals)) {
          ...
          log.ok(utiljs_substituteDegree(utiljs_substituteFraction(output)));
        }
      })(retval);
      ...
      listHelper($, 'h5', this, function() {
        log.ok('Group: ' + _.trim(utiljs_text(this)));
        var parent = {
...
var addProcedures = function($, obj) {
  ...
  listHelper($, '.instructions ol li[itemprop="recipeInstructions"] div', function(index, procedure) {
    ...
    if (header) {
      ...
      if (match) {
        header = ext_changeCase.titleCase(match[1]);
      }
    }
...
var addAsideNotes = function($, obj) {
  ...
  listHelper($, '.asides > .aside', function(i, aside) {
    listHelper($, 'h4', this, function() {
      h4 = _.trim(utiljs_text(this));
    });
    listHelper($, 'h3', this, function() {
      h3 = _.trim(utiljs_text(this));
    });
    ...
    $(this).children('p').each(function(index, element) {
      text = utiljs_substituteDegree(utiljs_substituteFraction(_.trim(utiljs_text(this))));
      if (text) {
    ...
    listHelper($, '.page-item', this, function() {
      ...
      listHelper($, 'figure > figcaption', this, function() {
        text = utiljs_substituteDegree(utiljs_substituteFraction(_.trim(utiljs_fulltext(this))));
        match = text.match(removeLeadingDigitPeriodRe);
        if (match) {
          text = match[1];
        }
...
var combineNotes = function(obj) {
  ...
  _.each(asideNotes, function(asideNote) {
    ...
    var intros = _.map(asideNote.intros, function(intro) {
      return '<p>' + intro + '</p>';
    }).join(utiljs_linefeed);

    if (intros && intros.length) {
      dest.push({
        text: title.concat(intros).join(utiljs_linefeed),
        order: i++,
        kind: 10
      });
    ...
    _.each(asideNote.notes, function(note) {
      noteObj = {
        text: title.concat(['<p>' + note.text + '</p>']).join(utiljs_linefeed),
        order: i++,
        kind: 10
      };
      title = []; // reset the <h4> and <h3>
