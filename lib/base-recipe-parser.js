...
var baserecipeparserjs_baserecipeparserjs = _backbone2.default.Model.extend({
  ...
  parseQuantity: function parseQuantity(text) {
    ...
    _util3.util.remove(matches, 0, 0);
    ...
    // retval[0] represents from (where there is a `to` in the quantity)
    // retval[1] represents to (where there is a `to` in the quantity)
    _util3.util.remove(retval, 2, 50);
    return retval;
  },
  ...
  getMeasurement: function getMeasurement(text) {
    ...
    if (altMeasurement) {
      ...
      if (_.indexOf(_util3.util.altMeasurementTerms, obj.matched.toLowerCase()) >= 0) {
