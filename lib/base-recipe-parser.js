...
_measurements = _.union(_measurements, _.map(_measurements, function(measurement) {
  return ext_pluralize_pluralize.plural(measurement);
})).sort();

var _uncountableWords = [
  'dozen', 'small', 'medium', 'large', 'mini', 'whole'
];
_uncountableWords.forEach(ext_pluralize_pluralize.addUncountableRule);
...
baserecipeparserjs_baserecipeparserjs = ext_backbone_Backbone.Model.extend({
  initialize: function() {
  ...
  chopWordsFromFront: function(text, array, from) {
    var tokenizer = new ext_natural_natural.RegexpTokenizer({pattern: wordTokenizerRe}),
        matched,
        found = 0;
  ...
  getMeasurement: function(text) {
    ...
    if (altMeasurement) {
      ...
      if (_.indexOf(utiljs_altMeasurementTerms, obj.matched.toLowerCase()) >= 0) {
  ...
  getDirectionsAndAlts: function(text) {
    var descriptionObj = this.getDescriptions(text),
        tokenizer = new ext_natural_natural.RegexpTokenizer({pattern: wordTokenizerRe}),
        tokenizerComma = new ext_natural_natural.RegexpTokenizer({pattern: directionTokenizerRe}),
