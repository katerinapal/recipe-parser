...
_measurements = _.union(_measurements, _.map(_measurements, function(measurement) {
  return pluralize_pluralize.plural(measurement);
})).sort();

var _uncountableWords = [
  'dozen', 'small', 'medium', 'large', 'mini', 'whole'
];
_uncountableWords.forEach(pluralize_pluralize.addUncountableRule);
...
baserecipeparserjs_baserecipeparserjs = backbone_Backbone.Model.extend({
  initialize: function() {
  ...
  parseQuantity: function(text) {
    ...
    // retval[0] represents from (where there is a `to` in the quantity)
    // retval[1] represents to (where there is a `to` in the quantity)
    util_utiljs.remove(retval, 2, 50);
    return retval;
  },
  ...
  chopWordsFromFront: function(text, array, from) {
    var tokenizer = new natural_natural.RegexpTokenizer({pattern: wordTokenizerRe}),
        matched,
        found = 0;
  ...
  getMeasurement: function(text) {
    ...
    if (altMeasurement) {
      ...
      if (_.indexOf(util_utiljs.altMeasurementTerms, obj.matched.toLowerCase()) >= 0) {
  ...
  getDirectionsAndAlts: function(text) {
    var descriptionObj = this.getDescriptions(text),
        tokenizer = new natural_natural.RegexpTokenizer({pattern: wordTokenizerRe}),
        tokenizerComma = new natural_natural.RegexpTokenizer({pattern: directionTokenizerRe}),
  ...
  getAllPieces: function(text) {
    var quantity = this.getQuantity(text),
        measurementObj = this.getMeasurement(text), // props: pruned, altMeasurement, matched
        descriptionObj = this.getDescriptions(text), // isOrSplit, descriptions, matchedDescriptions, parentheses, direction
        descriptions = descriptionObj.descriptions,
        directions = this.getDirectionsAndAlts(text), // [{direction, alt}[,{}, ...n]]
        quantities = [],
        measurementObjs = [];
    ...
    retval = _.zip.apply(_, _.map(_.zip(descriptions, quantities, measurementObjs, directions), function(tuple, idx) {
      ...
      if (idx > 0) { // already know the first has been parsed
...
export { baserecipeparserjs_baserecipeparserjs as baserecipeparserjs };
