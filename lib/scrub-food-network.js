import request_request from "request";
import cheerio_cheerio from "cheerio";
import changecase_changeCase from "change-case";
  ...
  listHelper($, '.ingredients > ul li', function(index, ingredient) {
    if (this.attr('itemprop') === 'ingredients') {
      ...
      if (group) {
        ...
        if (match) {
          group = changecase_changeCase.titleCase(match[1]);
        } else {
          group = changecase_changeCase.titleCase(group);
        }
      }
  ...
  listHelper($, '.directions[itemprop="recipeInstructions"] > *', function(index, procedure) {
    if (this.is('span.subtitle')) {
      ...
      if (header) {
        ...
        if (match) {
          header = changecase_changeCase.titleCase(match[1]);
        }
      }
...
scrape_scrape = function(callback, parser, url) {
  request_request(url, function (err, response, body) {
    if (err) { throw err; }
    var $ = cheerio_cheerio.load(body, {
      verbose: true,
    ...
    obj.parsedUrl = url_URL.parse(url, true);
